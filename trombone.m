...
%         end
    ...
    wvNextmh = uvNext(end-1) * quadIp(1) + uvNext(end) * quadIp(2) + wvNext(1) * quadIp(3);
...
%     upNext(end) = up(end) - rho * c * lambda ./ SBar(length(up)) .* (SHalf(length(up)) .* uvNextMph - SHalf(length(up) - 1) .* uvNext(end));
    ...
    potEnergyW(n) = rho / 2 * h * sum(SHalf(length(uv):end) .* wvNext .* wv .* potScalingW);
    ...
    % Plot the velocity
            subplot(3,1,1)
            hold off
            plot([hLocsLeft, hLocsLeft(end)+h] / L, [up;uPMp1], '-o', 'color', 'b');
            hold on;
            plot(hLocsRight / L, wp, '-o');
            scatter((hLocsLeft(end) + h)/L, uPMp1, 50, 'b', 'Marker', 'o')
            xlim([hLocsLeft(end-5)/L, hLocsRight(5)/L])
            ylim([-10, 10])
            subplot(3,1,2)
            hold off;
            plot(hLocsLeft / L + 0.5 / N, uvNext / amp, 'Marker', '.', 'MarkerSize', 10, 'Color', 'b');
            hold on;
            plot(hLocsRight / L - 0.5 / N, [wvNextmh; wvNext] / amp, 'Marker', '.', 'MarkerSize', 10,  'Color', 'r');
%             plot(hLocsLeft(end) / L + 0.5 / N, uvNextMph / amp, 'Marker', 'o', 'MarkerSize', 10, 'Color', 'r');
            scatter(hLocsRight(1) / L - 0.5 / N, wvNextmh / amp, 50, 'r', 'Marker', 'o');
            ylim([-10e-6, 10e-6])
